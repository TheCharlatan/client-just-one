                ...
                <Green
                    ...
                    onClick={async () => {
                        try {
                            ...
                            // TODO: Confirm that this way word get accepted.
                            await api.put(`game/${localStorage.getItem('gameId')}/${props.wordIndex}`, {headers: {'X-Auth-Token': requestHeader}});
                        }
                        catch (error) {
            ...
            <WordBox>
                <Green
                    ...
                    onClick={async () => {
                        try {
                            let requestHeader = 'X-Auth-Token ' + localStorage.getItem('token');
                            await api.delete(`game/${localStorage.getItem('gameId')}/${props.wordIndex}`, {headers: {'X-Auth-Token': requestHeader}});
                        }
                        catch (error) {
                            alert(`Something went wrong: \n${handleError(error)}`);
                        }
